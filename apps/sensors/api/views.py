...
class LatestHumidityView(APIView):
    ...
    def get(self, request):
        ...
        if not latest:
            return Response(
                {"data": {"value": None, "source": None, "timestamp": None}}, status=200
            )
...
def control_motor(request):
    ...
    try:
        ...
        logger.info(f"🎯 Motor control request received: state={state}")
        ...
        logger.info(f"✅ Motor state parsed: {state}")

        # Motor holatini o'zgartirish
        set_motor_state(state)
        logger.info(f"🔄 Motor state updated in backend")
        ...
        logger.info(f"📡 Motor state broadcast to 'humidity' group")
        ...
        logger.info(f"📡 Motor command sent to 'esp32_devices' group")
...
def get_motor_status(request):
    ...
    """
    return Response({"motor_state": get_motor_state()}, status=status.HTTP_200_OK)
